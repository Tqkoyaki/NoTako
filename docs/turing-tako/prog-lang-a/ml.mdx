...
1. It firstly gets ***type-checked*** by the compiler. The binding depended on a ***static environment*** (also refered to as the ***context***) which holds the types of all the preceding bindings in the file. A new static environment is then created with this new binding added.
