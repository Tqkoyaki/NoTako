...
You can use the `type()` function to check the type of value you are working with.
...
5. You can check the type of a variable by using the type function.
...
So far we have worked with the `print` method that prints out a value to a console. Python has a `input` method that similarly prints a value to the console but also waits for the user to input a value. The inputted value is always a String.
